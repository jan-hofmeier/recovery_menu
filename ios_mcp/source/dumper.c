...
int mlc_clone(int fsaHandle, int y_offset){
    ...
    gfx_printf(16, y_offset, 0, "MLC LBAs: 0x%llX", mlc_num_blocks);
    y_offset+=20;
...
int mlc_dump(int fsaHandle, int y_offset){
    ...
    do
    {
        ...
        if (write_result != buffer_size) {
            gfx_printf(20, y_offset + 20, GfxPrintFlag_ClearBG, "mlc: Failed to write %d bytes to file (result: %d)!", buffer_size, write_result);
            goto error;
        }         
        lba += buffer_size_lba;
...
void dump_nand_complete(int fsaHandle){
    ...
    slc_dump(fsaHandle, 175, "/vol/storage_recovsd/slc2.bin");
}